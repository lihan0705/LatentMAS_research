...
def build_agent_message_sequential_latent_mas(role: str, question: str, context: str = "", method=None, args=None):
    ...
    assert method in ["latent_mas", "latent_mas_tool"], "this prompt only for latent_mas method"
...
"""
            ...
            # For BFCL tool calling, the tool is called during latent reasoning
            # Judger should simply output the function call if tool was successful
            # or generate one if tool detection failed
            user_prompt = f"""Available Functions:
{context}

Target Question: {question}

You are provided with latent reasoning information.

Based on the reasoning, output the appropriate function call in the format: [CALL] function_name(param1=value1, param2=value2) [/CALL]
Do not provide any reasoning or explanation. Output ONLY the function call inside [CALL] and [/CALL] tags.
"""
...
def build_agent_messages_single_agent(question: str, args=None, item=None):

    task = args.task
    
    # BFCL standard: Use task-specific system message
    if task == "toolcalling":
        system_message = "You are an API calling assistant."
    else:
        system_message = "You are Qwen, created by Alibaba Cloud. You are a helpful assistant."
...
"""
        ...
        # BFCL v3 standard format - direct function string conversion
        functions_str = ""
        if item and "functions" in item:
            functions_str = "\n" + str(item["functions"])
        
        # BFCL standard prompt (no "reason step-by-step")
        user_content = f"""You are an API calling assistant. You are given a question and a set of available functions. 
Based on the question, you will need to make one or more function calls to help the user.
{functions_str}

Question: {question}

Make the function call(s) in the format: function_name(param1=value1, param2=value2)
"""
